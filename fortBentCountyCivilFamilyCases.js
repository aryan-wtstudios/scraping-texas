...
puppeteer
  ...
  .then(async (browser) => {
    ...
    await page.goto(
      "https://tylerpaw.fortbendcountytx.gov/PublicAccess/Search.aspx?ID=400"
    );

    await page.waitForSelector(
      "a.ssSearchHyperlink[href=\"javascript:LaunchSearch('Search.aspx?ID=400', false, true, sbxControlID2)\"]"
    );
    await page.click(
      "a.ssSearchHyperlink[href=\"javascript:LaunchSearch('Search.aspx?ID=400', false, true, sbxControlID2)\"]"
    );
    ...
    await page.type("#CaseSearchValue", "24-JEV21-29410");
    ...
    await page.goto(
      `https://tylerpaw.fortbendcountytx.gov/PublicAccess/${caseDetailUrl}`
    );
    ...
    const tableData = await page.evaluate(() => {
      ...
      return Array.from(rows).map((row) => {
        ...
        return Array.from(cells).map((cell) => cell.innerText.trim().replaceAll(',', ' ').replaceAll(';', ' ').replaceAll(/\s+/g, " "));
      });
    });
    ...
    fs.writeFile("fort_bent.csv", csvData, (err) => {
      if (err) {
